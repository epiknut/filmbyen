/**
 * Filmbyen master: nifty stuff
 * Version: 0.1
 */

$(function() {

	//only init masonry on bigger screens
	...
	//mobile menu
	$('.mobile, .royale-with-cheese, .line').on('click', function(e) {
		if(e.target != this) return; //prevent from affecting children of element
		e.preventDefault(); //prevent ghostclick
		var open 			= 'active',
				closing 	= 'closing';
		if ($('.mobile').hasClass(open)) {
			$('.mobile').removeClass(open);
			$('.dropdown').removeClass(open).addClass(closing).delay(300).queue(function() { 
				$(this).removeClass(closing).dequeue(); //due to unwanted transitions when scaling the website, we remove the class after the animation has finished (300ms)
			});
		} else { 
	...
	//mobile open sorting, no animation
	$('.shead.description').on('click', function(e) {
		e.preventDefault(); //prevent ghostclick
		var open 			= 'active';
		if ($('.sort').hasClass(open)) {
			$('.sort').removeClass(open);
			$('.sort h2 i').html('Vis filter<span class="squared"><span class="square-dot"></span><span class="square-dot last"></span></span>');
		} else { 
			$('.sort').addClass(open);
			$('.sort h2 i').html('Skjul filter<span class="squared"><span class="square-dot"></span><span class="square-dot last"></span></span>');
			//reinitialize jscrollpane to adjust width
	...
	//sort reviews
	$('.list-dropdown, .list-dropdown h3, .list-dropdown i').on('click', function(e) {
		if(e.target != this) return; //prevent from affecting children of element
		e.preventDefault(); //prevent ghostclick
		var open 		= 'active',
				closing = 'closing',
		...
		if(parent.hasClass(open)) {
			parent.removeClass(open).addClass(closing).delay(300).queue(function() { 
				$(this).removeClass(closing).dequeue(); //due to unwanted transitions when scaling the website, we remove the class after the animation has finished (300ms)
			});
		} else {
			$('.list-dropdown').parent().removeClass(open);
			parent.addClass(open);
		}
	});

	//close sorting menu on click outside of the dropdowns
	$(document).on('click', function(e) {
	  if (!$(e.target).closest('.sortables').length) {
	    $('.sortables').removeClass('active');
	  }
	});
